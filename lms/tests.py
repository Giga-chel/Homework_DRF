...
class LmsBaseTestCase(APITestCase):
    ...
    def setUp(self):
        # группы из фикстуры в тестовой базе нет — создаём вручную
...
class LessonCRUDTests(LmsBaseTestCase):
    ...
    def test_anonymous_cannot_create_lesson(self):
        response = self.client.post(
            ...
            {'course': self.course.pk, 'name': 'Урок'},
        )
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
    ...
    def test_user_sees_only_own_lessons(self):
        Lesson.objects.create(course=self.course, name='Урок постороннего', owner=self.stranger)
